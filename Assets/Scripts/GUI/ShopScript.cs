...
public class ShopScript : MonoBehaviour {
	...
	void Start () 
	{
		...
		hMenuScript = GameObject.Find("GUIGroup/MenuGroup").GetComponent<MenuScript>();
		hInGameController = (InGameController)GameObject.Find("Player").GetComponent(typeof(InGameController));
		hSoundController = (SoundController)GameObject.Find("SoundManager").GetComponent(typeof(SoundController));
		...
		iScrollState = 0;//vertical scroll initilization		
	...
	public void ShowMenu(int index)
	{
		...
		else if (index == (int)ShopMenus.Powerups)
		{
			...
			foreach (Transform costumeItem in currentTab)
				...
				{
					((ShopPowerupScript)costumeItem.GetComponent(typeof(ShopPowerupScript))).setShopPowerupScriptEnabled(true);
